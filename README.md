...
Local XCFramework builds require full Xcode with the iPhoneOS and iPhone
Simulator SDKs. Install the Homebrew tools and validate the selected Xcode
installation with:
...
The `boost-build` formula provides a current `b2` for development tooling.
Boost 1.65 release builds use the source-bundled `./b2` generated by
`bootstrap.sh`, because current Homebrew Boost.Build is not compatible with
Boost 1.65's `threadapi` feature declaration.
...
Test the published Boost 1.66.0 release:
