...
is included in the GitHub Actions job summary. A `prepare` run preserves those
exact artifacts and hands them to the publication workflow automatically.
...
| [1.83.0](https://www.boost.org/users/history/version_1_83_0.html) | C++20 | [![Boost 1.83.0 build](https://github.com/danoli3/ofxiOSBoost/actions/workflows/release-boost.yml/badge.svg)](https://github.com/danoli3/ofxiOSBoost/actions/workflows/release-boost.yml) | [Boost 1.83.0](https://github.com/danoli3/ofxiOSBoost/releases/tag/1.83.0) |
...
| [1.76.0](https://www.boost.org/users/history/version_1_76_0.html) | C++17 | [![Boost 1.76.0 build](https://github.com/danoli3/ofxiOSBoost/actions/workflows/release-boost.yml/badge.svg)](https://github.com/danoli3/ofxiOSBoost/actions/workflows/release-boost.yml) | [Boost 1.76.0](https://github.com/danoli3/ofxiOSBoost/releases/tag/1.76.0) |
...
Boost 1.76.0 remains on C++17 and introduces no new library. The smoke suite
adds portable Boost.Core bit/cmath coverage, offline Asio IP hashing, and
Boost.Bind interoperability with standard placeholders. Regex becomes
header-only for this C++17 build while its runtime matching test remains. The
complete suite passed on an arm64 iOS Simulator and physical arm64 iOS device.

Boost 1.77.0 remains on C++17 and adds the new header-only Boost.Describe and
Boost.Lambda2 libraries. The smoke suite also covers JSON fixed-array
conversion and hashing plus the opt-in Boost.Filesystem v4 path semantics.
The complete suite passed on an arm64 iOS Simulator and physical arm64 iOS
device.

Boost 1.78.0 remains on C++17 and introduces no new library. The smoke suite
adds coverage for Boost.Core span/type-name helpers, Boost.System result,
Variant2 visit-by-index, and updated Filesystem v4 trailing-separator semantics.
Three-slice compile/link and SwiftPM packaging checks pass. Simulator and
physical arm64 iOS device runtime suites both passed.

Boost 1.79.0 remains on C++17 and introduces no new library. The smoke suite
adds JSON Pointer, Unordered contains/erase-if, ThrowException
throw-with-location, and IO null-stream coverage. Three-slice compile/link and
...
Boost 1.81.0 remains on C++20 and adds compiled Boost.URL, Log, Container,
Timer, TypeErasure, and the portable Stacktrace basic backend. The smoke suite
adds URL parsing, Unordered flat-map, JSON described/optional conversion, and
System result-emplace coverage, plus separate one-per-frame tests for each new
compiled selection. Coroutine2 remains available header-only and runtime-tested
on the included Context archive. Three-slice compile/link checks pass, and the
...
Boost 1.84.0 remains on C++20 and adds compiled Boost.Cobalt to the static
XCFramework. Boost.Redis has no upstream Boost.Build archive; its
separate-compilation implementation and offline RESP3 request serialization
are compiled by the consumer test. Live Redis and TLS integration remain
deferred. Cobalt and Redis run as separate one-per-frame tests. The complete
three-slice package and generic Simulator/device Xcode app builds pass. The
complete sequential suite passes on an arm64 iOS Simulator and physical arm64
iOS device.

Boost 1.85.0 remains on C++20 and adds compiled Boost.Charconv, which is also
the new link dependency for offline Boost.MySQL coverage. Header-only
Boost.Scope receives separate scope-exit, success/fail, and unique-resource
tests. The official Boost.Container flat-map correctness patch is mandatory.
Additional one-per-frame regressions cover JSON, LEAF, Random, Core, PFR, and
Locale `char8_t`; Cobalt and offline Redis coverage carry forward. Build and
three-slice compile/link, standalone XCFramework, SwiftPM arm64/x86_64,
local CocoaPods, generic Simulator/device, and the complete arm64 Simulator
and physical arm64 device runtime suites pass.

Boost 1.86.0 remains on C++20 and adds no new upstream libraries. The compiled
1.85 set carries forward. Boost.Process v2 is newly compiled upstream but stays
excluded pending dedicated iOS process-spawning validation. Release-specific
coverage targets corrected Charconv parsing, Asio's deferred default token,
...
Boost 1.87.0 remains on C++20 and adds the new header-only Boost.Parser
library. Parser is packaged through the complete header tree and receives a
...
Boost 1.89.0 remains on C++20 and adds the new header-only Boost.Bloom
library. Bloom is packaged through the complete header tree and receives a
dedicated deterministic insert/lookup runtime test. The compiled 1.88 set
carries forward except System, which is header-only in 1.89 and no longer emits
...
Boost 1.91.0 remains on C++20 and adds the new header-only Boost.Decimal
library. Decimal is packaged through the complete header tree and receives
separate deterministic arithmetic and character-conversion coverage. Additional
1.91 tests cover Charconv negative 128-bit formatting, UUID character
conversion, URL query/decode conveniences, and System `unsafe_value`. The
compiled 1.90 set carries forward unchanged. Canonical packaging and the full
arm64 Simulator and physical arm64 device runtime suites pass.

Boost 1.92.0 remains on C++20 and introduces no newly accepted libraries. The
compiled 1.91 set carries forward unchanged. Dedicated deterministic coverage
targets Container `hub`, Decimal `decompose`, Lockfree's bounded ticket queue,
and System's corrected void-returning `unwrap_and_invoke` path. Canonical
three-slice, standalone, SwiftPM, CocoaPods, and complete arm64 Simulator and
physical arm64 device validation pass.
...
swift package add-dependency \
  https://github.com/danoli3/ofxiOSBoost.git --exact 1.92.0
...
select version `1.92.0`, and add the `ofxiOSBoost` product to the application
target.
...
```sh
BOOST_VERSION=1.92.0 ./example-swift-package/build.sh \
  dist/ofxiOSBoost-1.92.0.tar.gz
```
...
```sh
BOOST_VERSION=1.92.0 ./scripts/build-boost-ios.sh
```
...
```sh
./example-xcframework/build.sh dist/ofxiOSBoost-1.92.0.tar.gz
```
...
[Boost 1.92.0 upstream release](https://www.boost.org/releases/1.92.0/)

### Version 1.92.0 — August 12, 2026

Boost 1.92.0 remains on C++20 and introduces no newly accepted libraries. The
compiled library set carries forward unchanged. Release-specific tests cover
Container `hub`, Decimal `decompose`, Lockfree `bounded_ticket_queue`, and
System void-returning `unwrap_and_invoke`. Canonical three-slice, standalone,
SwiftPM, CocoaPods, and complete arm64 Simulator and physical arm64 device
validation pass.

[Boost 1.91.0 upstream release](https://www.boost.org/releases/1.91.0/)

### Version 1.91.0 — April 22, 2026

Boost 1.91.0 remains on C++20 and adds header-only Boost.Decimal. Dedicated
tests cover exact decimal arithmetic, deterministic character conversion,
Charconv negative 128-bit formatting, UUID character conversion, URL
query/decode conveniences, and System `unsafe_value`. The compiled 1.90 library
set carries forward unchanged. Canonical packaging and the full arm64 Simulator
and physical arm64 device runtime suites pass.
...
### Version 1.89.0 — August 14, 2025

Boost 1.89.0 remains on C++20, adds header-only Boost.Bloom, and makes
Boost.System header-only. Deterministic Bloom coverage, canonical packaging,
and the complete arm64 Simulator and physical arm64 device suites pass.
...
### Version 1.87.0 — December 12, 2024
...
### Version 1.86.0 — August 14, 2024

Boost 1.86.0 remains on C++20 and carries forward the compiled 1.85 set.
...
### Version 1.85.0 — April 15, 2024

Boost 1.85.0 remains on C++20, adds compiled Charconv and header-only Scope,
and applies the official Container flat-map correctness patch. Canonical
three-slice package and consumer validation passes, as do the complete arm64
Simulator and physical arm64 device runtime suites.

[Boost 1.84.0 upstream release history](https://www.boost.org/users/history/version_1_84_0.html)

### Version 1.84.0 — December 13, 2023

Boost 1.84.0 remains on C++20 and adds compiled Cobalt plus Redis offline
separate-compilation coverage. The complete sequential suite passes on an
arm64 iOS Simulator and physical arm64 iOS device.
...
### Version 1.81.0 — December 14, 2022

Boost 1.81.0 remains on C++20 and adds compiled Boost.URL, Log, Container,
Timer, TypeErasure, and the portable Stacktrace basic backend. Coverage expands
to URL parsing, Unordered flat-map, JSON described/optional conversion, System
result emplacement, and a separate sequential runtime test for each new
compiled selection. Three-slice compile/link and SwiftPM packaging checks pass,
and the complete expanded suite passed on an arm64 iOS Simulator and physical
arm64 iOS device.
...
### Version 1.79.0 — April 13, 2022

Boost 1.79.0 remains on C++17 and adds no new library. Coverage expands to
JSON Pointer, Unordered contains/erase-if, ThrowException throw-with-location,
and the IO null stream. Three-slice compile/link and SwiftPM packaging checks
pass; the complete runtime suite passed on an arm64 iOS Simulator and physical
arm64 iOS device.

[Boost 1.78.0 upstream release history](https://www.boost.org/users/history/version_1_78_0.html)

### Version 1.78.0 — December 8, 2021

Boost 1.78.0 remains on C++17 and adds no new library. Coverage expands to
Boost.Core span/type-name helpers, Boost.System result, Variant2
visit-by-index, and updated Filesystem v4 trailing-separator semantics.
Three-slice compile/link and SwiftPM packaging checks pass; the complete runtime
suite passed on an arm64 iOS Simulator and physical arm64 iOS device.

[Boost 1.77.0 upstream release history](https://www.boost.org/users/history/version_1_77_0.html)

### Version 1.77.0 — August 5, 2021

Boost 1.77.0 remains on C++17 and adds Boost.Describe and Boost.Lambda2
coverage, JSON fixed-array conversion and hashing, and opt-in Filesystem v4
path semantics. The complete suite passed on an arm64 iOS Simulator and
physical arm64 iOS device.

[Boost 1.76.0 upstream release history](https://www.boost.org/users/history/version_1_76_0.html)

### Version 1.76.0 — April 16, 2021

Boost 1.76.0 remains on C++17. It adds Boost.Core bit/cmath, Asio IP hashing,
and Bind standard-placeholder coverage. Regex is header-only in this C++17
configuration. The complete suite passed on an arm64 iOS Simulator and
physical arm64 iOS device.
