...
BOOST_VERSION="${BOOST_VERSION:-1.74.0}"
...
if [[ "$BOOST_VERSION" == "1.65.0" || "$BOOST_VERSION" == "1.66.0" || "$BOOST_VERSION" == "1.67.0" || "$BOOST_VERSION" == "1.68.0" || "$BOOST_VERSION" == "1.69.0" || "$BOOST_VERSION" == "1.70.0" || "$BOOST_VERSION" == "1.71.0" || "$BOOST_VERSION" == "1.72.0" || "$BOOST_VERSION" == "1.73.0" || "$BOOST_VERSION" == "1.74.0" ]]; then
    DEFAULT_BOOST_LIBS="$DEFAULT_BOOST_LIBS context"
fi
if [[ "$BOOST_VERSION" == "1.73.0" || "$BOOST_VERSION" == "1.74.0" ]]; then
...
case "$BOOST_VERSION" in
    1.61.0|1.62.0|1.63.0|1.64.0|1.65.0|1.66.0|1.67.0|1.68.0|1.69.0|1.70.0|1.71.0|1.72.0|1.73.0|1.74.0) ;;
...
HOST_TOOLSET=cc
if [[ "$BOOST_VERSION" == "1.71.0" || "$BOOST_VERSION" == "1.72.0" || "$BOOST_VERSION" == "1.73.0" || "$BOOST_VERSION" == "1.74.0" ]]; then
    HOST_TOOLSET=clang
fi
(
    cd "$SOURCE_DIR"
...
using darwin : iosdevice
    : $CLANGXX
    : <compileflags>"-arch arm64 -isysroot $IPHONEOS_SDK -miphoneos-version-min=$IOS_MIN_VERSION -fPIC -DBOOST_SP_USE_SPINLOCK -std=$CPPSTD -stdlib=libc++ -fvisibility=hidden -fvisibility-inlines-hidden -Wno-deprecated-declarations -Wno-deprecated-builtins -Wno-unknown-warning-option"
...
using darwin : iossimulatorarm64
    : $CLANGXX
    : <compileflags>"-arch arm64 -isysroot $SIMULATOR_SDK -mios-simulator-version-min=$IOS_MIN_VERSION -fPIC -DBOOST_SP_USE_SPINLOCK -std=$CPPSTD -stdlib=libc++ -fvisibility=hidden -fvisibility-inlines-hidden -Wno-deprecated-declarations -Wno-deprecated-builtins -Wno-unknown-warning-option"
...
using darwin : iossimulatorx86_64
    : $CLANGXX
    : <compileflags>"-arch x86_64 -isysroot $SIMULATOR_SDK -mios-simulator-version-min=$IOS_MIN_VERSION -fPIC -DBOOST_SP_USE_SPINLOCK -std=$CPPSTD -stdlib=libc++ -fvisibility=hidden -fvisibility-inlines-hidden -Wno-deprecated-declarations -Wno-deprecated-builtins -Wno-unknown-warning-option"
...
build_platform() {
    ...
    if [[ ( "$BOOST_VERSION" == "1.70.0" || "$BOOST_VERSION" == "1.71.0" || "$BOOST_VERSION" == "1.72.0" || "$BOOST_VERSION" == "1.73.0" || "$BOOST_VERSION" == "1.74.0" ) && " $BOOST_LIBS " == *" locale "* ]]; then
