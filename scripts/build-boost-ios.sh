...
if [[ "$BOOST_VERSION" == "1.89.0" || "$BOOST_VERSION" == "1.90.0" || "$BOOST_VERSION" == "1.91.0" || "$BOOST_VERSION" == "1.92.0" ]]; then
    # Boost.System's Boost.Build target became a header-only alias in 1.89.
    DEFAULT_BOOST_LIBS="${DEFAULT_BOOST_LIBS/ system/}"
fi
...
if [[ "$BOOST_VERSION" == "1.76.0" || "$BOOST_VERSION" == "1.77.0" || "$BOOST_VERSION" == "1.78.0" || "$BOOST_VERSION" == "1.79.0" || "$BOOST_VERSION" == "1.80.0" || "$BOOST_VERSION" == "1.81.0" || "$BOOST_VERSION" == "1.82.0" || "$BOOST_VERSION" == "1.83.0" || "$BOOST_VERSION" == "1.84.0" || "$BOOST_VERSION" == "1.85.0" || "$BOOST_VERSION" == "1.86.0" || "$BOOST_VERSION" == "1.87.0" || "$BOOST_VERSION" == "1.88.0" || "$BOOST_VERSION" == "1.89.0" || "$BOOST_VERSION" == "1.90.0" || "$BOOST_VERSION" == "1.91.0" || "$BOOST_VERSION" == "1.92.0" ]]; then
...
if [[ "$BOOST_VERSION" == "1.81.0" || "$BOOST_VERSION" == "1.82.0" || "$BOOST_VERSION" == "1.83.0" || "$BOOST_VERSION" == "1.84.0" || "$BOOST_VERSION" == "1.85.0" || "$BOOST_VERSION" == "1.86.0" || "$BOOST_VERSION" == "1.87.0" || "$BOOST_VERSION" == "1.88.0" || "$BOOST_VERSION" == "1.89.0" || "$BOOST_VERSION" == "1.90.0" || "$BOOST_VERSION" == "1.91.0" || "$BOOST_VERSION" == "1.92.0" ]]; then
    DEFAULT_BOOST_LIBS="$DEFAULT_BOOST_LIBS url log container timer type_erasure stacktrace"
fi
if [[ "$BOOST_VERSION" == "1.84.0" || "$BOOST_VERSION" == "1.85.0" || "$BOOST_VERSION" == "1.86.0" || "$BOOST_VERSION" == "1.87.0" || "$BOOST_VERSION" == "1.88.0" || "$BOOST_VERSION" == "1.89.0" || "$BOOST_VERSION" == "1.90.0" || "$BOOST_VERSION" == "1.91.0" || "$BOOST_VERSION" == "1.92.0" ]]; then
...
if [[ "$BOOST_VERSION" == "1.85.0" || "$BOOST_VERSION" == "1.86.0" || "$BOOST_VERSION" == "1.87.0" || "$BOOST_VERSION" == "1.88.0" || "$BOOST_VERSION" == "1.89.0" || "$BOOST_VERSION" == "1.90.0" || "$BOOST_VERSION" == "1.91.0" || "$BOOST_VERSION" == "1.92.0" ]]; then
...
case "$BOOST_VERSION" in
    1.61.0|1.62.0|1.63.0|1.64.0|1.65.0|1.66.0|1.67.0|1.68.0|1.69.0|1.70.0|1.71.0|1.72.0|1.73.0|1.74.0|1.75.0|1.76.0|1.77.0|1.78.0|1.79.0|1.80.0|1.81.0|1.82.0|1.83.0|1.84.0|1.85.0|1.86.0|1.87.0|1.88.0|1.89.0|1.90.0|1.91.0|1.92.0) ;;
...
if [[ "$BOOST_VERSION" == "1.83.0" || "$BOOST_VERSION" == "1.84.0" || "$BOOST_VERSION" == "1.85.0" || "$BOOST_VERSION" == "1.86.0" || "$BOOST_VERSION" == "1.87.0" || "$BOOST_VERSION" == "1.88.0" || "$BOOST_VERSION" == "1.89.0" || "$BOOST_VERSION" == "1.90.0" || "$BOOST_VERSION" == "1.91.0" || "$BOOST_VERSION" == "1.92.0" ]]; then
    case "$BOOST_VERSION" in
        ...
        1.87.0) EXPECTED_SOURCE_SHA="af57be25cb4c4f4b413ed692fe378affb4352ea50fbe294a11ef548f4d527d89" ;;
        1.88.0) EXPECTED_SOURCE_SHA="46d9d2c06637b219270877c9e16155cbd015b6dc84349af064c088e9b5b12f7b" ;;
        1.89.0) EXPECTED_SOURCE_SHA="85a33fa22621b4f314f8e85e1a5e2a9363d22e4f4992925d4bb3bc631b5a0c7a" ;;
        1.90.0) EXPECTED_SOURCE_SHA="49551aff3b22cbc5c5a9ed3dbc92f0e23ea50a0f7325b0d198b705e8ee3fc305" ;;
        1.91.0) EXPECTED_SOURCE_SHA="de5e6b0e4913395c6bdfa90537febd9028ea4c0735d2cdb0cd9b45d5f51264f5" ;;
        1.92.0) EXPECTED_SOURCE_SHA="5c1d40cb8e19adbf740a4ec2da35b3e58f3f5804b1dce44deb53df72193cbc6c" ;;
    esac
    ACTUAL_SOURCE_SHA="$(shasum -a 256 "$SOURCE_ARCHIVE" | awk '{ print $1 }')"
    if [[ "$ACTUAL_SOURCE_SHA" != "$EXPECTED_SOURCE_SHA" ]]; then
        echo "Boost $BOOST_VERSION source checksum mismatch." >&2
...
case "$BOOST_VERSION" in
    1.92.0)
        # Dry-run validated against the checksum-verified B2 5.5.3 release.
        COMPAT_PATCH="$REPO_ROOT/patches/boost-1.85.0-build-engine.patch"
        ;;
    1.91.0)
        # Dry-run validated against the checksum-verified B2 5.3.2 release.
        COMPAT_PATCH="$REPO_ROOT/patches/boost-1.85.0-build-engine.patch"
        ;;
    1.90.0)
        # Boost 1.90 retains the B2 5.2.1/Locale compatibility points.
        COMPAT_PATCH="$REPO_ROOT/patches/boost-1.85.0-build-engine.patch"
        ;;
    1.89.0)
        # Boost 1.89 retains the B2 5.2.1/Locale compatibility points.
        COMPAT_PATCH="$REPO_ROOT/patches/boost-1.85.0-build-engine.patch"
        ;;
    1.88.0)
        # Boost 1.88 retains the B2 5.2.1/Locale compatibility points.
        COMPAT_PATCH="$REPO_ROOT/patches/boost-1.85.0-build-engine.patch"
        ;;
    1.87.0)
        # Boost 1.87 retains the B2 5.2.1/Locale compatibility points.
        COMPAT_PATCH="$REPO_ROOT/patches/boost-1.85.0-build-engine.patch"
        ;;
    1.86.0)
    ...
    1.85.0)
        ...
        ;;
    1.84.0)
        # Boost 1.84 retains the B2/iOS compatibility points from 1.83.
        COMPAT_PATCH="$REPO_ROOT/patches/boost-1.81.0-build-engine.patch"
        ;;
    1.83.0)
        # Dry-run validated against the updated B2 4.10.1 release.
        COMPAT_PATCH="$REPO_ROOT/patches/boost-1.81.0-build-engine.patch"
        ;;
    1.82.0)
        # Boost 1.82 retains the same tested iOS compatibility points as 1.81.
        COMPAT_PATCH="$REPO_ROOT/patches/boost-1.81.0-build-engine.patch"
        ;;
    1.81.0)
    ...
    1.80.0)
        ...
        ;;
    1.79.0)
        # Boost 1.79 retains the same iOS compatibility points as 1.78.
        COMPAT_PATCH="$REPO_ROOT/patches/boost-1.78.0-build-engine.patch"
        ;;
    1.78.0)
...
if [[ "$BOOST_VERSION" == "1.85.0" ]]; then
    echo "Applying official Boost.Container 1.85 flat_map correctness fix"
    patch -d "$SOURCE_DIR" -p0 < \
        "$REPO_ROOT/patches/boost-1.85.0-container-flat-map.patch"
fi
if [[ "$BOOST_VERSION" == "1.83.0" ]]; then
    echo "Applying official Boost.Unordered 1.83 copy-assignment fix"
    patch -d "$SOURCE_DIR" -p1 < \
        "$REPO_ROOT/patches/boost-1.83.0-unordered-copy-assignment.patch"
fi
...
HOST_TOOLSET=cc
if [[ "$BOOST_VERSION" == "1.71.0" || "$BOOST_VERSION" == "1.72.0" || "$BOOST_VERSION" == "1.73.0" || "$BOOST_VERSION" == "1.74.0" || "$BOOST_VERSION" == "1.75.0" || "$BOOST_VERSION" == "1.76.0" || "$BOOST_VERSION" == "1.77.0" || "$BOOST_VERSION" == "1.78.0" || "$BOOST_VERSION" == "1.79.0" || "$BOOST_VERSION" == "1.80.0" || "$BOOST_VERSION" == "1.81.0" || "$BOOST_VERSION" == "1.82.0" || "$BOOST_VERSION" == "1.83.0" || "$BOOST_VERSION" == "1.84.0" || "$BOOST_VERSION" == "1.85.0" || "$BOOST_VERSION" == "1.86.0" || "$BOOST_VERSION" == "1.87.0" || "$BOOST_VERSION" == "1.88.0" || "$BOOST_VERSION" == "1.89.0" || "$BOOST_VERSION" == "1.90.0" || "$BOOST_VERSION" == "1.91.0" || "$BOOST_VERSION" == "1.92.0" ]]; then
    HOST_TOOLSET=clang
fi
(
    cd "$SOURCE_DIR"
...
build_platform() {
    ...
    if [[ "$BOOST_VERSION" == "1.65.0" || "$BOOST_VERSION" == "1.66.0" || "$BOOST_VERSION" == "1.67.0" || "$BOOST_VERSION" == "1.68.0" || "$BOOST_VERSION" == "1.69.0" || "$BOOST_VERSION" == "1.70.0" || "$BOOST_VERSION" == "1.71.0" || "$BOOST_VERSION" == "1.72.0" || "$BOOST_VERSION" == "1.73.0" || "$BOOST_VERSION" == "1.74.0" || "$BOOST_VERSION" == "1.75.0" || "$BOOST_VERSION" == "1.76.0" || "$BOOST_VERSION" == "1.77.0" || "$BOOST_VERSION" == "1.78.0" || "$BOOST_VERSION" == "1.79.0" || "$BOOST_VERSION" == "1.80.0" || "$BOOST_VERSION" == "1.81.0" || "$BOOST_VERSION" == "1.82.0" || "$BOOST_VERSION" == "1.83.0" || "$BOOST_VERSION" == "1.84.0" || "$BOOST_VERSION" == "1.85.0" || "$BOOST_VERSION" == "1.86.0" || "$BOOST_VERSION" == "1.87.0" || "$BOOST_VERSION" == "1.88.0" || "$BOOST_VERSION" == "1.89.0" || "$BOOST_VERSION" == "1.90.0" || "$BOOST_VERSION" == "1.91.0" || "$BOOST_VERSION" == "1.92.0" ]]; then
        local abi=sysv
    ...
    if [[ ( "$BOOST_VERSION" == "1.70.0" || "$BOOST_VERSION" == "1.71.0" || "$BOOST_VERSION" == "1.72.0" || "$BOOST_VERSION" == "1.73.0" || "$BOOST_VERSION" == "1.74.0" || "$BOOST_VERSION" == "1.75.0" || "$BOOST_VERSION" == "1.76.0" || "$BOOST_VERSION" == "1.77.0" || "$BOOST_VERSION" == "1.78.0" || "$BOOST_VERSION" == "1.79.0" || "$BOOST_VERSION" == "1.80.0" || "$BOOST_VERSION" == "1.81.0" || "$BOOST_VERSION" == "1.82.0" || "$BOOST_VERSION" == "1.83.0" || "$BOOST_VERSION" == "1.84.0" || "$BOOST_VERSION" == "1.85.0" || "$BOOST_VERSION" == "1.86.0" || "$BOOST_VERSION" == "1.87.0" || "$BOOST_VERSION" == "1.88.0" || "$BOOST_VERSION" == "1.89.0" || "$BOOST_VERSION" == "1.90.0" || "$BOOST_VERSION" == "1.91.0" || "$BOOST_VERSION" == "1.92.0" ) && " $BOOST_LIBS " == *" locale "* ]]; then
